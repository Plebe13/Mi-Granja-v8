...
const STABLE_LEVELS = {
  ...
  4: {
    ...
    bonusMilk: 0.3,
    cost: { coins: 220, madera: 50, hierro: 8, low_gem: 1 }
  }
};
...
const PIG_PEN_LEVELS = {
  ...
  4: {
    ...
    cost: { coins: 200, madera: 50, hierro: 8, low_gem: 1 }
  }
};
...
export function renderCorrales() {
  ...
  const coopNext  = COOP_LEVELS[coopLevel + 1] || null;
  const h         = game.house || {};
  ...
  // Lista gallinas simple
  const henList = gallinas.length
  ...
  // 🐔 GALLINAS
    ...
    bodyHtml = `
      <div class="card" style="margin-bottom:8px;padding:6px 8px;font-size:.85rem;line-height:1.25">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
          <span style="font-weight:600">🏚️ Gallinero</span>
          <span style="font-size:.8rem">Nivel ${coopLevel}</span>
        </div>

        <div style="display:flex;justify-content:space-between;font-size:.8rem;margin-bottom:2px">
          <span>Capacidad: <strong>${coopCfg.capacity}</strong> gallinas</span>
          <span>Huevos almacenados: <strong>${h.coopEggs || 0}</strong></span>
        </div>
        ...
        ${
          coopNext
            ? `
              <div style="border-top:1px solid rgba(255,255,255,.08);margin-top:4px;padding-top:4px;font-size:.78rem;display:flex;justify-content:space-between;align-items:center;gap:6px;flex-wrap:wrap">
                <div>
                  <div><strong>Próxima mejora:</strong> Nivel ${coopLevel + 1}</div>
                  <div style="font-size:.75rem">Capacidad: ${coopNext.capacity} gallinas</div>
                  <div style="font-size:.75rem">Coste: ${renderCost(coopNext.cost)}</div>
                </div>
                <button class="btn xsmall" id="btn-upgrade-coop"
                  ${!canPayCost(coopNext.cost) ? 'disabled' : ''}>
                  Mejorar gallinero
                </button>
              </div>
            `
            : `
              <div style="margin-top:2px;font-size:.75rem">
                Tu gallinero ya está al máximo nivel.
              </div>
            `
        }
      </div>
      ...
      <div class="card">
        ...
        <p class="kv">Cantidad: <strong>${gallinas.length}</strong> / ${coopCfg.capacity}</p>
        ${henList}

        <div style="margin-top:8px;margin-bottom:4px">
          <button class="btn small" id="btn-feed-hens" ${feedDisabledAttr}>Alimentar gallinas (maíz)</button>
        </div>
        ${
          autoFeed
            ? `<p class="kv" style="font-size:.75rem">La alimentación automática está activa. Desactívala en <strong>Mi casa → Gallinero</strong> para usar este botón.</p>`
            : ''
        }
        <div style="margin-bottom:8px">
          <button class="btn small" id="btn-collect-eggs-manual">Recoger huevos (manual)</button>
        </div>
        ...
        <p class="kv">
          Nivel actual: <strong>${coopLevel}</strong> · Capacidad: <strong>${coopCfg.capacity}</strong> gallinas.
        </p>
        <p class="kv">
        ...
        <p class="kv">Precio: ${BUY_PRICES.gallina.coins} ₥ cada una.</p>
  ...
  // Botón de mejora de gallinero
  const btnCoop = el.querySelector('#btn-upgrade-coop');
  if (btnCoop && coopNext) {
    btnCoop.addEventListener('click', () => {
      ...
      if (!canPayCost(nextCfg.cost)) {
        toast('No tienes suficientes recursos para mejorar el gallinero.');
        return;
      }
...
function bindCorralesEvents() {
  ...
  // Gallinas: alimentar (sólo si autoFeed está OFF)
  root.querySelector('#btn-feed-hens')?.addEventListener('click', () => {
    ...
    let fed = 0;
    hens.forEach(h => {
      if (game.inv.maiz > 0 && !h.fedToday) { // no gastar maíz dos veces
        game.inv.maiz -= 1;
        h.fedToday = true;
        fed++;
      }
  ...
  // Gallinas: sacrificar individual (funciona siempre, sin importar la edad)
  root.querySelectorAll('.btn-hen-cull').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const meatGain = 1; // luego podemos hacerlo según edad/peso
      game.inv.meat = (game.inv.meat || 0) + meatGain;
  ...
  if (autoFeed && hens.length) {
    for (const h of hens) {
      if ((game.inv.maiz || 0) > 0) {
        game.inv.maiz -= 1;
        h.fedToday = true;   // se queda en true para el HUD
      }
    }
  }
  ...
  for (const h of hens) {
    ...
    if (fed && h.alive !== false) {
      if (autoCollect && level >= 3) {
        // huevo va directo al depósito del gallinero
        game.house.coopEggs = (game.house.coopEggs || 0) + 1;
      } else {
        // queda en el nido de esa gallina para recogida manual
        h.eggs = (h.eggs || 0) + 1;
      }
    }

    // Sacrificio automático por vejez (nivel 5 + autoCull ON)
    if (autoCull && h.alive !== false && (h.age || 0) >= 30) {
      h.alive = false;
      game.inv.meat = (game.inv.meat || 0) + 2; // un poco mejor recompensa
    }
  }
